...
const imagesDir = path.join(path.resolve(), 'upload');
 ...
 */
const fileAllow = ['png', 'jpg', 'jpeg', 'svg', 'gif', 'avif', 'webp', 'mp3', 'wav', 'wma', 'aac', 'flac', 'ogg', 'm4a', 'amr', 'mp4', 'avi', 'mov', 'wmv', 'flv', 'mkv', 'webm', 'm4v'];
export const fileUp = async link => {
  try {
    ...
    if (!fileAllow.includes(ext.toLowerCase())) throw new Error('Invalid file extension.');// Check if file extension is valid
    ...
    const filePath = path.join(imagesDir, fileName); // Write image file to images directory
    fs.writeFileSync(filePath, buffer);
    return `upload/${fileName}`;// Return file path
  } catch (e) {
    console.error(e);
