  ...
  return {
    ...
    isEmail: (mail) => {
      ...
      if (!pattern.test(mail)) {
        (resultEmail = false), (messageEmail = 'Invalid mail');
      }
    ...
    checkPass: (pass) => {
      if (pass?.length <= 5) {
        (resultPass = false), (messagePass = 'Password less then 6');
      }
    ...
    checkConfirmPass: (pass) => {
      ...
      if (!isPassOk) {
        (confirmPass = false), (messageConfirmPass = 'password not match');
      }
